        ...
        <div className="bg-background/80 backdrop-blur-xl rounded-2xl w-full max-w-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="p-5 space-y-4">
            <h4 className="text-sm font-medium text-foreground/80">
              Создати {type === "text" ? "текстовую палату" : "голосовую палату"}
            </h4>
            <input autoFocus value={value} onChange={(e) => onChange(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") onSave(); if (e.key === "Escape") onClose(); }}
              placeholder={type === "text" ? "общая-бесѣда" : "Общiй гласъ"}
              className="w-full bg-foreground/5 rounded-xl px-3 py-2.5 text-sm text-foreground placeholder:text-foreground/20 outline-none focus:ring-1 focus:ring-foreground/20" />
            <div className="flex items-center justify-end gap-2">
              ...
              <button onClick={onSave} disabled={!value.trim()} className="px-4 py-2 rounded-xl bg-foreground/10 hover:bg-foreground/15 text-foreground/80 text-sm transition-colors disabled:opacity-30">Создати</button>
            </div>
          </div>
        </div>
      </div>
