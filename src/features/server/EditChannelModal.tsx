      ...
      <div className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center" onClick={onClose}>
        <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="px-5 py-4 border-t border-border flex items-center justify-end gap-2">
            ...
            <button onClick={onSave} disabled={!name.trim()} className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium transition-colors">Сохранити</button>
          </div>
        </div>
      </div>
    </Portal>
