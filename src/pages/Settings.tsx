...
import { Crown, Edit2, Image, Check, X, LogOut, Headphones, Palette, Keyboard, User } from "lucide-react";
...
export function Settings() {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row bg-background min-w-0">
      ...
      <div className="flex-1 overflow-y-auto md:border-t-0">
        <div className="max-w-2xl mx-auto p-4 md:p-6 space-y-4 md:space-y-6">
          ...
          {activeTab === "profile" && (
            <>
              {/* Banner */}
              <div className={`relative h-32 rounded-lg overflow-hidden bg-gradient-to-br ${currentBanner.gradient} border border-primary/20 group`}>
                <div className="absolute inset-0 opacity-20" style={{ backgroundImage: currentBanner.pattern }} />
      ...
      {showBannerSelector && (
        <BannerSelector currentBanner={selectedBanner} onSelect={handleBannerSelect} onClose={() => setShowBannerSelector(false)} />
      )}
    </div>
  );
}
