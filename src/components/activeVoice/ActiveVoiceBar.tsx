...
export function ActiveVoiceBar() {
  ...
  const [userDataMap, setUserDataMap] = useState<Record<string, { username: string; avatarUrl?: string }>>({});
    ...
    Promise.all(
      missing.map(async (id) => {
        try {
          ...
          return { id, data: { username: user.username, avatarUrl: user.avatar_url } };
        } catch {
  ...
  return (
    ...
    >
      <button
        onClick={() => console.log("=== HOTKEY DEBUG ===", {
      ...
      <div data-debug-hotkeys className="hidden text-[10px] text-muted-foreground font-mono px-2">
          ...
          return (
            <div key={p.identity} className="-mr-2 last:mr-0">
              <UserPopover
                ...
                onVolumeChange={(v) => dispatch(setParticipantVolume({ identity: p.identity, volume: v }))}
              >
                <div
                  className={`w-7 h-7 rounded-full border-2 border-background flex items-center justify-center text-xs font-bold overflow-hidden cursor-pointer ${
                    p.isSpeaking ? "ring-2 ring-primary" : p.isMuted ? "opacity-60" : ""
                  } ${p.isLocal ? "bg-violet-600 text-white" : "bg-muted text-foreground"}`}
                >
                  {ud?.avatarUrl ? (
                    <img src={ud.avatarUrl} alt="" className="w-full h-full object-cover" />
                  ) : (
